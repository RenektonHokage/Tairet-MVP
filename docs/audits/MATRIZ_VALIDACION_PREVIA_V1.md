...
| MVP-MAT-79 | Selección congelada del tercer bloque de rollout de `F7` | Código + SQL observado + docs operativos | Si ya puede fijarse que `reviews` entra en `F7 CODE 03` y que cualquier otra tabla o flow queda explícitamente fuera del tercer rollout | `infra/sql/migrations/013_create_reviews.sql:10`, `infra/sql/migrations/013_create_reviews.sql:32`, `infra/sql/migrations/013_create_reviews.sql:35`, `functions/api/src/server.ts:64`, `functions/api/src/routes/reviews.ts:77`, `functions/api/src/routes/reviews.ts:114`, `functions/api/src/routes/reviews.ts:158`, `apps/web-b2c/src/hooks/useReviews.ts:162`, `apps/web-b2c/src/hooks/useReviews.ts:232`, `docs/audits/STATUS.md`, `docs/audits/RIESGOS_ACEPTADOS_Y_PENDIENTES.md` | Confirmado por código + SQL observado + docs operativos |
| MVP-MAT-80 | Validación post-apply de `F7 CODE 03` sobre `reviews` | Código + SQL observado + entorno real | Si el tercer rollout aplicado a `reviews` quedó efectivamente endurecido en Supabase sin expandirse a otras tablas o flows y sin romper los flows reales del bloque | `infra/sql/rls.sql:12`, `infra/sql/rls.sql:25`, `infra/sql/migrations/013_create_reviews.sql:32`, `infra/sql/migrations/013_create_reviews.sql:35`, `infra/sql/migrations/018_harden_reviews_rls_backend_only.sql:1`, `functions/api/src/services/supabase.ts:12`, `functions/api/src/routes/reviews.ts:77`, `functions/api/src/routes/reviews.ts:114`, `functions/api/src/routes/reviews.ts:158`, `apps/web-b2c/src/hooks/useReviews.ts:162`, `apps/web-b2c/src/hooks/useReviews.ts:232`; verificación real post-apply en Supabase: `reviews_select_public` ya no existe, `reviews_select_backend_only` existe para `anon` y `authenticated` con `qual = false`; smoke funcional mínimo real: `GET /reviews` y `POST /reviews` siguen funcionando | Confirmado por código + runtime/entorno real |
| MVP-MAT-81 | Reevaluación del cuarto bloque prudente de rollout de `F7` | Código + SQL observado + docs operativos | Si después de `F7 CODE 01`, `F7 CODE 02` y `F7 CODE 03` existe un cuarto bloque con blast radius suficientemente bajo como para habilitar otro rollout inmediato | `docs/audits/STATUS.md`, `docs/audits/RIESGOS_ACEPTADOS_Y_PENDIENTES.md`, `infra/sql/rls.sql:31`, `infra/sql/rls.sql:41`, `infra/sql/rls.sql:83`, `infra/sql/migrations/006_create_catalog_tables.sql:7`, `infra/sql/migrations/006_create_catalog_tables.sql:26`, `functions/api/src/routes/public.ts:367`, `functions/api/src/routes/public.ts:384`, `functions/api/src/routes/orders.ts:137`, `functions/api/src/routes/payments.ts:24`, `functions/api/src/middlewares/panelAuth.ts:109` | No habilitado / pausa operativa |
| MVP-MAT-82 | Selección del primer bloque de `F8` | Docs operativos + riesgos + smoke tests | Si ya puede fijarse que el bloque inicial de `F8` queda limitado a cierre documental, backlog no bloqueante y clasificación de `/panel/metrics` como diferido / no activo, sin habilitar CODE ni reabrir `F3`, `F6` o `F7` | `docs/audits/STATUS.md`, `docs/audits/RIESGOS_ACEPTADOS_Y_PENDIENTES.md`, `docs/audits/SMOKE_TESTS_V1.md`, `docs/audits/HARDENING_ROADMAP.md` | Confirmado por docs operativos |
| MVP-MAT-83 | Reevaluación de un segundo bloque útil de `F8` | Docs operativos + riesgos + smoke tests | Si después del primer bloque docs-only de `F8` existe otro bloque útil de cierre documental / backlog no bloqueante que justifique trabajo inmediato, sin reabrir `F3`, `F6` o `F7` | `docs/audits/STATUS.md`, `docs/audits/RIESGOS_ACEPTADOS_Y_PENDIENTES.md`, `docs/audits/SMOKE_TESTS_V1.md`, `docs/audits/HARDENING_ROADMAP.md` | No habilitado / sin más trabajo inmediato |
...
4. El siguiente paso operativo de `F7` pasa a ser:
   ...
   * `orders`, `reservations`, `panel_users`, `payment_events`, `locals`, `local_daily_ops`, `ticket_types`, `table_types` y los flows derivados de alta criticidad (`payments/callback`, `check-in`, `orders/search`, `orders/summary`, `export`, bootstrap auth panel) quedan explícitamente fuera del tercer rollout
   * la validación de `F7 CODE 03` queda respaldada por `MVP-MAT-80` (repo/SQL + estado real post-apply en Supabase + smoke funcional mínimo del bloque)
   * que `reviews` siga fuera de `schema.sql` debe leerse como deuda documental separada dentro del drift SQL del repo; no reabre `F7 CODE 03` ni debe volver a reclasificar este slice como validado parcial
   ...
   * el siguiente paso ya no es abrir `F7 CODE 04` ni revalidar `F7 CODE 03`, sino esperar nueva evidencia o replanteo de rollout antes de reevaluar el remanente de `F7`

5. Tratar el primer bloque de `F8` como docs-only / backlog no bloqueante:
   ...
   * el bloque inicial de `F8` ya queda consolidado / cumplido como cierre documental del ciclo actual, no como rollout técnico
   ...
   * lo restante se mantiene separado entre backlog no bloqueante, frentes pausados y trabajo técnico no habilitado; no corresponde abrir un segundo bloque activo de `F8`

6. Mantener esta matriz como referencia obligatoria antes de:

   * abrir nuevos ASK sensibles
   * redactar prompts CODE
   * tocar contratos o flujos críticos

7. Reemitir esta matriz en una siguiente versión solo cuando cambie uno de estos estados:
