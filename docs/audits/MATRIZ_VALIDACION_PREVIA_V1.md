...
5. Tratar el primer bloque de `F8` como docs-only / backlog no bloqueante:
   ...
   * el bloque inicial de `F8` ya queda seleccionado como cierre documental del ciclo actual, no como rollout técnico
   * el siguiente paso operativo de `F8` es consolidar backlog no bloqueante y sincronización documental, no abrir CODE nuevo
