...
- `B5b` no queda cerrado completo: siguen pendientes `ticket_types`, `table_types`, blast radius de `SUPABASE_SERVICE_ROLE` y drift SQL/env.
...
- en ese momento, `reservations` seguia pendiente; el checkpoint `6.8` documenta su cierre posterior;
- `B5b` no quedaba cerrado completo: seguian pendientes `ticket_types`, `table_types`, blast radius de `SUPABASE_SERVICE_ROLE`, drift SQL/env y `/payments/callback` si aplica.
...
- en ese momento, `locals` y `reservations` seguian pendientes; los checkpoints `6.7` y `6.8` documentan sus cierres posteriores;
- `B5b` no quedaba cerrado completo: seguian pendientes `ticket_types`, `table_types`, blast radius de `SUPABASE_SERVICE_ROLE`, drift SQL/env y `/payments/callback` si aplica.
...
### 6.8 Checkpoint `B5b-6 reservations`

- `public.reservations` fue remediado como siguiente slice de High-Risk Data Exposure Containment y versionado en `infra/sql/migrations/022_harden_reservations_data_api.sql`;
...
- la tabla contiene PII y datos operativos de reservas: `name`, `last_name`, `email`, `phone`, `date`, `guests`, `status`, `notes`, `table_note`, `created_at` y `updated_at`;
- el cambio aplicado y validado en Supabase live mantiene RLS on, elimina `reservations_insert_public` / `reservations_select_by_local` y remueve grants de `anon` / `authenticated`;
...
- en ese momento quedaban como sub-slices reducir DTOs/selects en `POST /orders`, reducir DTOs/selects en `POST /reservations`, reemplazar `select("*")` en mutaciones panel puntuales y validar `localId` en `GET /events/whatsapp_clicks/count`; los checkpoints `6.12`, `6.13` y `6.14` documentan el cierre posterior de los DTO/selects publicos, las mutaciones panel puntuales y la validacion de tracking;
...
- `PATCH /panel/reservations/:id` y `PATCH /panel/orders/:id/use` fueron remediados como sub-slice de Panel mutation selects hardening;
- el cambio reduce blast radius del backend con `SUPABASE_SERVICE_ROLE` en mutaciones puntuales del panel, pero no elimina `SUPABASE_SERVICE_ROLE` ni cambia reglas de negocio;
- `PATCH /panel/reservations/:id` redujo el fetch interno a `id`, `local_id`, `status`, `email`, `name`, `date` y `guests`, conservando tenant check, validacion de estado y datos necesarios para email;
- `PATCH /panel/reservations/:id` redujo la respuesta/update DTO a `id`, `status`, `table_note` y `updated_at`, sin devolver PII ni `local_id`;
- `PATCH /panel/orders/:id/use` redujo el fetch interno a `id`, `local_id`, `status`, `used_at`, `valid_from`, `valid_to`, `is_window_legacy` y `created_at`, conservando tenant check y validacion de ventana;
- `PATCH /panel/orders/:id/use` redujo la respuesta/update DTO a `id`, `status` y `used_at`, sin devolver PII, `checkin_token`, `items`, `transaction_id` ni campos internos de ventana;
- `PATCH /panel/checkin/:token` no fue modificado porque es el flujo operativo real del scanner y ya usa selects explicitos para los datos que muestra;
- `apps/web-next/lib/reservations.ts` quedo alineado con `PanelReservationMutationResponse`; el tipo `Reservation` usado por listados no fue modificado;
- verificaciones registradas: `pnpm -C functions/api typecheck` OK y `pnpm -C apps/web-next typecheck` OK;
- QA live aprobado: confirmar reserva, cancelar reserva, editar `table_note`, nota visible en UI, listar/buscar reservas, email de confirmacion/cancelacion, check-in QR/token por `PATCH /panel/checkin/:token`, scanner con datos necesarios, `GET /panel/orders/search`, `GET /panel/orders/summary`, `GET /activity`, `GET /metrics/summary` y calendario month/day;
- no se tocaron Data API containment, RLS, grants, policies, SQL, migraciones, endpoints publicos, payments/callback ni service role;
- `B5b` no queda cerrado completo: siguen pendientes drift SQL/env y `/payments/callback` si pagos reales aplican.

### 6.14 Checkpoint `B5b-12 GET /events/whatsapp_clicks/count validation`

- `GET /events/whatsapp_clicks/count` fue remediado como sub-slice de tracking validation;
- el cambio reduce ruido y queries invalidas en un endpoint publico que opera via backend privilegiado con `SUPABASE_SERVICE_ROLE`, pero no elimina `SUPABASE_SERVICE_ROLE`;
- se agrego `whatsappClickCountQuerySchema` con `localId: z.string().uuid()` en `functions/api/src/routes/events.ts`;
- el endpoint ahora usa `safeParse(req.query)` y responde `400` si `localId` falta o no es UUID valido;
- para UUIDs validos mantiene el contrato `{ local_id: localId, count: count ?? 0 }`;
...
- no se toco `POST /events/whatsapp_click`, metrics, activity, frontend, SQL, RLS, grants, policies, migraciones, payments/callback ni service role;
