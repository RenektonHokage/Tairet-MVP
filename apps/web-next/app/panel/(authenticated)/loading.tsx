export default function PanelLoading() {
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <div className="h-9 w-56 rounded-lg bg-neutral-200/70 animate-pulse" />
        <div className="h-4 w-80 max-w-full rounded bg-neutral-200/60 animate-pulse" />
      </div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div
            key={`panel-kpi-${index}`}
            className="h-[112px] rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"
          >
            <div className="h-4 w-28 rounded bg-neutral-200/70 animate-pulse" />
            <div className="mt-4 h-8 w-20 rounded bg-neutral-200/70 animate-pulse" />
            <div className="mt-3 h-3 w-32 rounded bg-neutral-200/60 animate-pulse" />
          </div>
        ))}
      </div>

      <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <div className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
          <div className="h-5 w-40 rounded bg-neutral-200/70 animate-pulse" />
          <div className="mt-5 h-[260px] rounded-2xl bg-neutral-200/60 animate-pulse" />
        </div>
        <div className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
          <div className="h-5 w-32 rounded bg-neutral-200/70 animate-pulse" />
          <div className="mt-5 space-y-3">
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={`panel-list-${index}`} className="h-10 rounded-lg bg-neutral-200/60 animate-pulse" />
            ))}
          </div>
        </div>
      </div>
    </div>
  );
}
