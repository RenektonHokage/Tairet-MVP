...
import { parseAccessPaidCheckinToken } from "@/lib/accessCheckin";
import {
  Card,
  CardContent,
  CardDescription,
...
type ScannerStatus =
  ...
  | "no_camera"
  | "scanner_error";
type ScannerMessageTone = "neutral" | "danger";

interface AccessPaidQrScannerProps {
  disabled?: boolean;
  onTokenDetected: (token: string) => void | Promise<void>;
}
...
function getScannerStatusLabel(status: ScannerStatus): string {
  switch (status) {
    ...
    case "processing":
      return "QR detectado. Buscando entrada...";
    case "paused_after_result":
      return "Scanner pausado. Revisá el resultado antes de escanear otro QR.";
    case "permission_denied":
      return "No se pudo acceder a la cámara. Podés usar el input manual.";
    case "no_camera":
      return "No se detectó cámara disponible. Usá el input manual.";
    case "scanner_error":
      return "No se pudo iniciar el scanner.";
  }
}

export function AccessPaidQrScanner({
  disabled = false,
  onTokenDetected,
}: AccessPaidQrScannerProps) {
  ...
  const showScannerMessage = useCallback(
    (message: string, durationMs?: number, tone: ScannerMessageTone = "neutral") => {
      clearNoticeTimeout();
      setScannerMessageTone(tone);
  ...
  const handleDecodedText = useCallback(
    async (decodedText: string) => {
      ...
      const parsedToken = parseAccessPaidCheckinToken(decodedText);
      if (!parsedToken.ok) {
        invalidQrCooldownUntilRef.current = now + INVALID_QR_COOLDOWN_MS;
        showScannerMessage(
          "Código no válido para entrada pagada.",
          INVALID_QR_COOLDOWN_MS,
          "danger"
        );
        return;
      }
      ...
      try {
        ...
        showScannerMessage("QR detectado. Revisá el resultado antes de validar.");
      } catch {
        showScannerMessage(
          "No pudimos buscar la entrada. Reintentá o usá el input manual.",
          undefined,
          "danger"
        );
      } finally {
  ...
  const startCamera = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.enumerateDevices) {
      ...
      setScannerMessage("No se detectó cámara disponible. Usá el input manual.");
      return;
    }
    ...
    try {
      ...
      if (errorName === "NotAllowedError" || errorName === "SecurityError") {
        ...
        setScannerMessage("No se pudo acceder a la cámara. Podés usar el input manual.");
        return;
      }
  ...
  const canStopCamera =
    ...
    scannerStatus === "scanning" ||
    scannerStatus === "processing";
  const canScanAnother = !disabled && scannerStatus === "paused_after_result";
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Usá la cámara para leer el QR de una entrada pagada. La validación requiere
          confirmación manual.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded-xl border border-neutral-200 bg-neutral-950 p-2">
          <div className="relative overflow-hidden rounded-lg bg-black">
            <video
              aria-label="Vista previa de cámara para escanear QR de entrada pagada"
              autoPlay
              className={cn(
        ...
        <div className="space-y-2">
          ...
          {scannerMessage ? (
            <p
              className={cn(
                "text-sm font-medium",
                scannerMessageTone === "danger" ? "text-rose-700" : "text-neutral-700"
              )}
            >
              {scannerMessage}
            </p>
