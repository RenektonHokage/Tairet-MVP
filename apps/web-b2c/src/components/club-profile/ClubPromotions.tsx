...
interface ClubPromotionsProps {
  ...
  localId: string;
  localSlug?: string; // Slug del local para mapeo de promos (ej: "morgan", "celavie", "dlirio")
}
  ...
  const handleOpenPromo = (promo: Promotion) => {
    ...
    if (!promo || !localId) {
      if (import.meta.env.DEV) {
        console.log("[ClubPromotions] handleOpenPromo: missing promo or localId", { promo, localId });
      }
      return;
    }
    
    if (import.meta.env.DEV) {
      console.log("[ClubPromotions] handleOpenPromo: clicked", { promoTitle: promo.title, promoId: promo.id, localId, localSlug });
    }
    
    // Obtener UUID real de la promo (mapeo de localSlug:título a UUID de DB)
    const realPromoId = getRealPromoId({ localSlug, localId, title: promo.title, fallbackId: promo.id });
    
    if (import.meta.env.DEV) {
      console.log("[ClubPromotions] handleOpenPromo: resolved promoId", { original: promo.id, resolved: realPromoId });
    }
    
    // trackPromoOpen valida internamente si es UUID válido y envía el request
    void trackPromoOpen(localId, realPromoId, "club_promo_card");
  };

  if (!promotions || promotions.length === 0) {
    return (
      <section className="space-y-4 sm:space-y-6">
        <h2 className="text-xl sm:text-2xl font-bold text-foreground">Promos del local</h2>
